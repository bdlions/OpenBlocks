...
/**
 * PartialCodeDisplayer stores and displays user-generated text that
 ...
 * appropriate.
 *
 * text : String //the text stored in this PartialCodeDisplayer and edited by the user
 */
public class PartialCodeDisplayer extends JPanel {
    ...
    /**Manager for arrow drawn from this to parent while in editing mode**/
  	...
  	/** The commentLabel linked to this PartialCodeDisplayer and placed on the commentSource 	 */
     ...
     * Swing methods OR through the convenience method PartialCodeDisplayer.setParent()
     *
     ...
     */
    public PartialCodeDisplayer(String initText, CommentSource source, Color borderColor, double zoom){
    	...
    	textArea.addMouseListener(new MouseAdapter() {
    		...
    		public void mouseEntered(MouseEvent e) {			
    			PartialCodeDisplayer comment = PartialCodeDisplayer.this;
    ...
    public void reformComment(){
    	int w = textArea.isEditable() ? (int)(this.width*zoom) : (int)(PartialCodeDisplayer.MINIMUM_WIDTH*zoom);
    	int h = textArea.isEditable() ? (int)(this.height*zoom) : (int)(PartialCodeDisplayer.MINIMUM_HEIGHT*zoom);
    ...
    public String getSaveString(){
        ...
        saveString.append("<PartialCodeDisplayer>");
     ...
     * @return
     */
    public static PartialCodeDisplayer loadComment( NodeList commentChildren, RenderableBlock rb) {
        PartialCodeDisplayer comment = null;
        boolean commentCollapsed = false;
        
        Node commentChild;
        String text = null;
        Point commentLoc = new Point(0,0);
        Dimension boxSize = new Dimension(PartialCodeDisplayer.DEFAULT_WIDTH, PartialCodeDisplayer.DEFAULT_HEIGHT);
        ...
        for(int j=0; j<commentChildren.getLength(); j++){
            ...
            if(commentChild.getNodeName().equals("Text")){
            	...
            	System.out.println("Uknown PartialCodeDisplayer Node: " + commentChild.getNodeName());
            }
        }

        if(text != null){
            comment = new PartialCodeDisplayer(text, rb, rb.getBlock().getColor(), rb.getZoom());
     ...
     */
 	public void paint(Graphics g){
 		...
 		g2.fill(body);
 		if (active) {
 			g2.setColor(PartialCodeDisplayer.background.brighter());
 		} else {
     ...
     */
     public String toString() {
        return "PartialCodeDisplayer ID: " + " at " + this.getLocation() + " with text: \"" + getText() + "\"";
    }
      ...
      * outer PartialCodeDisplayer class.
      */
     private class CommentEventListener implements FocusListener, MouseListener, MouseMotionListener{
    	 ...
    	 public void mouseMoved(MouseEvent e) {
    		 if(textArea.isEditable()){
	    		 if(e.getX()>(width-2*margin) && e.getY()>(height-2*margin)){
	    			 PartialCodeDisplayer.this.setCursor(new Cursor(Cursor.SE_RESIZE_CURSOR));
	    		 }else{
    	 ...
    	 public void mousePressed(MouseEvent e) {
    		 PartialCodeDisplayer.this.grabFocus();  //atimer.stop();
    		 showOnTop();
	 ...
	 */
	 public static void main(String[] args) {
		 ...
		 /**
			...
			p.add(new PartialCodeDisplayer("", null,  Color.green, 1.0), 0);
			f.add(p);
			p.add(new PartialCodeDisplayer("", null,  Color.orange, 1.0), 0);
			f.add(p);
			p.add(new PartialCodeDisplayer("", null,  Color.red, 1.0), 0);
			f.add(p);
			p.add(new PartialCodeDisplayer("", null,  Color.blue, 1.0), 0);
			f.add(p);
			f.setVisible(true);
		  */
	 }
